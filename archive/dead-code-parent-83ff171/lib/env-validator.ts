...
const ENV_CONFIG: EnvConfig = {
  ...
  optional: [
    { name: 'JWT_SECRET', description: 'JWT signing secret (or NEXTAUTH_SECRET); required in production for auth' },
...
export function validateEnvironment(useCache: boolean = true): ValidationResult {
  ...
  if (process.env.NODE_ENV === 'production' && !process.env.JWT_SECRET && !process.env.NEXTAUTH_SECRET) {
    warnings.push('JWT_SECRET or NEXTAUTH_SECRET should be set in production for secure authentication')
  }
