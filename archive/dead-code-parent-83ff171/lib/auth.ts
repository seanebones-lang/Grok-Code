...
// JWT Configuration - no fallback in production to avoid insecure default
const JWT_SECRET = process.env.JWT_SECRET || process.env.NEXTAUTH_SECRET || (process.env.NODE_ENV === 'production' ? '' : 'fallback-secret-change-in-production')
const JWT_EXPIRY = process.env.JWT_EXPIRY || '7d'

if (process.env.NODE_ENV === 'production' && !JWT_SECRET) {
  console.warn('[auth] JWT_SECRET or NEXTAUTH_SECRET must be set in production. JWT operations will throw.')
}
 ...
 * @throws In production if JWT_SECRET/NEXTAUTH_SECRET is not set
 */
export function signJWT(payload: { userId: string; email?: string; name?: string }): string {
  if (!JWT_SECRET) {
    throw new Error('JWT_SECRET or NEXTAUTH_SECRET must be set for signing tokens')
  }
  return jwt.sign(
    {
...
/**
 * Verify JWT token
 * @returns Decoded payload or null if invalid/missing secret
 */
export function verifyJWT(token: string): { userId: string; email?: string; name?: string } | null {
  if (!JWT_SECRET) {
    console.warn('[auth] JWT_SECRET not set; verification will fail')
    return null
  }
  try {
