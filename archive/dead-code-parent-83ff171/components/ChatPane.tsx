...
export function ChatPane({ repository, newSessionMessage, onNewSessionHandled }: ChatPaneProps) {
  ...
  // Use ref to only run once per distinct message and avoid double-send when deps change
  const lastHandledSessionMessageRef = useRef<string | null>(null)
  useEffect(() => {
    if (!newSessionMessage) {
      lastHandledSessionMessageRef.current = null
      return
    }
    if (newSessionMessage === lastHandledSessionMessageRef.current) return
    lastHandledSessionMessageRef.current = newSessionMessage
    clearMessages()
    setError(null)
